import glob
import os
import xml.etree.ElementTree as ET

import pandas as pd

os.environ['PYTHONPATH'] += ':models/research/:models/research/slim'
PATH_PROJECT = os.path.abspath(os.path.join(os.getcwd(), os.pardir, os.pardir))
PATH_DATASET = os.path.abspath(os.path.join(os.getcwd(),  'Dataset'))


def xml_to_csv(path):
    xml_list = []
    for xml_file in glob.glob(path + '/*.xml'):
        tree = ET.parse(xml_file)
        root = tree.getroot()
        for member in root.findall('object'):
            value = (root.find('filename').text,
                     int(root.find('size')[0].text),
                     int(root.find('size')[1].text),
                     member[0].text,
                     int(member[4][0].text),
                     int(member[4][1].text),
                     int(member[4][2].text),
                     int(member[4][3].text)
                     )
            xml_list.append(value)
    column_name = ['filename', 'width', 'height', 'class', 'xmin', 'ymin', 'xmax', 'ymax']
    xml_df = pd.DataFrame(xml_list, columns=column_name)
    return xml_df


def main(directory_list):
    base_path = PATH_DATASET
    for Image_cat in directory_list:
        image_path = os.path.join(base_path, 'Images/{}'.format(Image_cat))
        xml_df = xml_to_csv(image_path)
        xml_df.to_csv(os.path.join(base_path, 'data/{}_labels.csv'.format(Image_cat)), index=None)
        print('Successfully converted xml to csv.')


if __name__ == '__main__':
    main(['train', 'test'])
